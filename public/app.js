...
function medByName(name) {
  return meds.find(m => m.name.toLowerCase() === (name || '').toLowerCase());
}

function medOptions(listId) {
  return `<datalist id="${listId}">
    ${meds.map(m => `<option value="${esc(m.name)}"></option>`).join('')}
  </datalist>`;
}
...
function makeRow(data = {}) {
  ...
  $('medicineRows').appendChild(div);
  updatePreview();
}

function refreshDatalists() {
  document.querySelectorAll('.med-row').forEach(row => {
    const old = row.querySelector('datalist');
    if (old) {
      old.innerHTML = meds.map(m => `<option value="${esc(m.name)}"></option>`).join('');
    }
  });
}

function getItems() {
  return [...document.querySelectorAll('.med-row')].map(r => ({
    name: r.querySelector('.m-name').value.trim(),
    quantity: r.querySelector('.m-qty').value.trim(),
    days: r.querySelector('.m-days').value.trim(),
    dose: r.querySelector('.m-dose').value.trim(),
    route: r.querySelector('.m-route').value.trim(),
    instruction: r.querySelector('.m-ins').value.trim()
  })).filter(x => x.name);
}
  ...
  $('pv_items').innerHTML = getItems().map((it, i) => `
    <div class="rx-item">
      <div class="rx-item-name">
        ...
        <span class="qty">Số lượng: ${esc(it.quantity || '………………')}</span>
      </div>
...
function fillMedForm(m) {
  $('newMedName').value = m.name || '';
  $('newMedQty').value = m.default_quantity || '';
  $('newMedDays').value = m.default_days || '';
  $('newMedDose').value = m.default_dose || '';
  $('newMedRoute').value = m.default_route || '';
  $('newMedIns').value = m.default_instruction || '';
}

function clearMedForm() {
  ['newMedName', 'newMedQty', 'newMedDays', 'newMedDose', 'newMedRoute', 'newMedIns'].forEach(id => $(id).value = '');
}

async function loadMedicines() {
  meds = await fetch('/api/medicines').then(r => r.json());
  refreshDatalists();
  renderMedicineList();
}
...
function renderMedicineList() {
  $('medicineList').innerHTML = meds.map(m => `
    <div class="medicine-item">
      <div>
        <b>${esc(m.name)}</b><br>
        <small>SL: ${esc(m.default_quantity || '')} | Ngày: ${esc(m.default_days || '')} lần | Mỗi lần: ${esc(m.default_dose || '')} | ${esc(m.default_route || '')}</small>
      </div>
      <div class="medicine-actions">
        <button type="button" class="secondary small" onclick='fillMedForm(${jsString(m)})'>Sửa</button>
        <button type="button" class="danger small" onclick='deleteMedicine(${m.id}, ${JSON.stringify(m.name)})'>Xóa</button>
      </div>
    </div>
  `).join('') || '<div class="medicine-item">Chưa có thuốc trong danh mục.</div>';
}

async function deleteMedicine(id, name) {
  if (!confirm(`Xóa thuốc "${name}" khỏi danh mục thuốc tham khảo?`)) return;

  const r = await fetch('/api/medicine/' + id, { method: 'DELETE' });
  const j = await r.json();

  $('status').textContent = j.ok ? 'Đã xóa thuốc khỏi danh mục.' : (j.error || 'Lỗi xóa thuốc');

  if (j.ok) await loadMedicines();
}

['patient_name', 'gender', 'age', 'diagnosis', 'advice', 'prescription_date']
  .forEach(id => $(id).addEventListener('input', updatePreview));

$('prescription_date').value = todayISO();
$('advice').value = DEFAULT_ADVICE;

$('addRow').onclick = () => makeRow();

$('newBtn').onclick = () => {
  document.querySelectorAll('.main-panel input, .main-panel textarea, .main-panel select').forEach(el => {
    if (el.id !== 'prescription_date') el.value = '';
  });
...
$('patientSelect').onchange = e => {
  ...
  $('advice').value = p.note || DEFAULT_ADVICE;
  updatePreview();
};
...
$('saveBtn').onclick = async () => {
  ...
  const j = await r.json();
  $('status').textContent = j.ok ? 'Đã lưu toa, bệnh nhân và thuốc mới.' : (j.error || 'Lỗi lưu');
  await loadHistory();
refreshPatients();
  await loadMedicines();
...
$('saveMedBtn').onclick = async () => {
  const payload = {
    ...
    default_quantity: $('newMedQty').value,
    default_days: $('newMedDays').value,
    default_dose: $('newMedDose').value,
    default_route: $('newMedRoute').value,
    default_instruction: $('newMedIns').value
  };
  ...
  const j = await r.json();
  $('status').textContent = j.ok ? 'Đã lưu thuốc vào danh mục thuốc tham khảo. Có thể chọn ngay trong ô Tên thuốc.' : (j.error || 'Lỗi lưu thuốc');
...
async function loadHistory() {
  ...
  $('history').innerHTML = list.slice(0, 50).map(x => `
    <div class="history-item" onclick="loadPrescription(${x.id})" style="cursor:pointer">
      <b>${esc(x.patient_name)}</b><br>
      ${esc(x.prescription_date || '')} - ${esc(x.diagnosis || '')}
    </div>
...
async function loadPrescription(id){
  ...
  $('patient_name').value = p.patient_name || '';
  $('gender').value = p.gender || '';
  $('age').value = p.age || '';
  $('diagnosis').value = p.diagnosis || '';
  $('advice').value = p.advice || '';
  $('prescription_date').value = p.prescription_date || todayISO();
...
async function refreshPatients(){
  ...
  $('patientSelect').innerHTML =
    ...
    patients.map(p =>
      `<option value='${JSON.stringify(p).replace(/'/g,"&#39;")}'>${p.name} ${p.age ? '- ' + p.age + ' tuổi' : ''}</option>`
    ).join('');
}
