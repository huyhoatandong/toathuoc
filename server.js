...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
  ...
  console.error('Hãy tạo file .env khi chạy local, hoặc thêm Environment Variables khi deploy online.');
  process.exit(1);
}
...
async function ensureSampleMedicines() {
  ...
  if (error) {
    console.error('Không kiểm tra được bảng medicines:', error.message);
    return;
  }
  ...
  await supabase.from('medicines').insert([
    {
      ...
      default_quantity: '10 viên',
      default_days: '3',
      default_dose: '1 viên',
      default_route: 'Uống sau ăn',
      default_instruction: 'Khi đau/sốt'
    },
    {
      name: 'Omeprazol 20mg',
      default_quantity: '14 viên',
      default_days: '1',
      default_dose: '1 viên',
      default_route: 'Uống trước ăn 30 phút',
      default_instruction: ''
    },
    {
      name: 'Cefixim 200mg',
...
app.get('/api/prescriptions', async (req, res, next) => {
  try {
    ...
    const rows = (data || []).map(x => ({
      ...x,
      items_json: JSON.stringify(x.items || [])
    }));
...
app.post('/api/prescription', async (req, res, next) => {
  try {
    ...
    const items = Array.isArray(p.items)
      ? p.items.filter(x => x && String(x.name || '').trim())
      : [];
    ...
    for (const it of items) {
      ...
      await supabase.from('medicines').upsert({
        ...
        default_days: it.days || '',
        default_dose: it.dose || '',
        default_route: it.route || '',
        default_instruction: it.instruction || '',
        updated_at: new Date().toISOString()
      }, { onConflict: 'name' });
    }
...
app.listen(PORT, () => {
  console.log('');
  console.log('==============================================');
  console.log(` Toa thuốc tự túc đang chạy: http://localhost:${PORT}`);
  console.log(' Dữ liệu đang lưu online bằng Supabase.');
